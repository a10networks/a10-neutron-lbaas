...
def status_update(a10_driver):
    ...
    for lb in lb_db.get_loadbalancers(context):
        lb_h = handler_lb.LoadbalancerHandler(a10_driver,
                                              a10_driver.openstack_driver.load_balancer,
                                              neutron=a10_driver.neutron)
        lb_h.oper(context, lb, lb_db, models.LoadBalancer)
        for pool in lb.pools:
            pool_h = handler_pool.PoolHandler(a10_driver,
                                              a10_driver.openstack_driver.pool,
                                              neutron=a10_driver.neutron)
